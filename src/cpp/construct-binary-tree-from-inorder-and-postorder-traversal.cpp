...
public:
    TreeNode *buildTree(vector<int> &inorder, vector<int> &postorder) 
    {
    	...
    	int debug = 0;
		TreeNode* root =  innerBuildTree(inorder,postorder,0,inorder.size()-1,0,postorder.size()-1,&debug);

		return root;

    }
    TreeNode *innerBuildTree(vector<int> &inorder, vector<int> &postorder,int astart,int aend,int bstart,int bend ,int* debug) 
    {
        TreeNode* node = NULL;
        (*debug)++;
        int root = getRoot(postorder,bend);
        ...
        if ( astart <= aend &&  bstart<= bend )
        {
	        ...
	        int start2 = bstart;
	        int end2 = bstart + leftLen - 1;
	        if ( start1 <= end1 && start2 <= end2  )
	        	node->left = innerBuildTree(inorder,postorder,start1,end1,start2,end2,debug);
	        
	        start1 = leftsize + 1;
	        end1 = aend;
	        start2 = bstart+leftLen;
	        end2 = start2 + rightLen - 1;

	        if ( start1 <= end1 && start2 <= end2  )
	        	node->right = innerBuildTree(inorder,postorder,start1,end1,start2,end2,debug);
	    }
		return node;
    }
...
int main(int argc, char const *argv[])
{
	...
	vector<int> v4;
	TreeNode* newRoot = s.buildTree(v1,v2);
	//printf("%d\n", newRoot->val);
	prevorder(newRoot,v4);
	printVector1(v4);
